{/* Upgrades Modal */}
   {showUpgradesModal && (
     <div className="fixed inset-0 bg-black bg-opacity-70 z-50 flex items-center justify-center p-4">
       <div className="bg-gray-800 rounded-lg p-4 max-w-4xl w-full max-h-[70vh] flex flex-col">
               ...
               >
                 <div className="flex justify-between items-center mb-1">
                   <div className="flex items-center">
                     <span className="bg-gray-700 text-xs px-2 py-1 rounded mr-2">{index + 1}</span>
       ...
       <div className="bg-gray-800 rounded-lg p-4 flex flex-col">
         ...
         {ownedAgents.length === 0 ? (
           ...
           <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 mb-4">
             {/* Sort agents by autonomy to maintain consistent order with the shop */}
             {ownedAgents
               .sort((a, b) => {
                 const aType = getAgentType(a.type);
                 const bType = getAgentType(b.type);
                 return aType.autonomy - bType.autonomy;
               })
               .map(agent => {
                 const agentType = getAgentType(agent.type);
                 const isInVerificationMode = agentsInVerificationMode.includes(agent.id);

                 return (
                   <div
                     key={agent.id}
                     className={`${
                       isInVerificationMode ? 'bg-blue-900' : 'bg-gray-700'
                     } p-3 rounded-lg border ${isInVerificationMode ? 'border-blue-500' : 'border-gray-600'}`}
                   >
                     <div className="flex justify-between items-center">
                       ...
                       <span className={`text-xs px-2 py-1 rounded ${
                         ...
                         {isInVerificationMode ? 'Verifying' : `${agent.currentTasks}/${agentType.maxTasks}`}
                       </span>
                     </div>
                     ...
                     {/* Show verification mode toggle button for high-autonomy agents */}
                     {agentType.autonomy >= 0.8 && (
                       <button
                         onClick={() => toggleVerificationMode(agent.id)}
                         className={`w-full mt-2 py-1 px-2 rounded text-xs ${
                           isInVerificationMode
                             ? 'bg-blue-600 hover:bg-blue-700'
                             : 'bg-gray-600 hover:bg-gray-500'
                         }`}
                       >
                         {isInVerificationMode ? 'Back to Work' : 'Verify Other Models'}
                       </button>
                     )}
                   </div>
                 );
         ...
         <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-2">
           {AI_AGENT_TYPES
             // Sort by autonomy for consistent ordering
             .sort((a, b) => a.autonomy - b.autonomy)
             .map(agentType => {
               const currentCount = getAgentCount(agentType.id);
               const isMaxedOut = currentCount >= agentType.maxCount;

               return (
                 <div
                   key={agentType.id}
                   className={`border rounded-md p-3 ${
                     isMaxedOut ? 'bg-gray-800 border-gray-700' :
                     money >= agentType.cost ? 'border-blue-600' : 'border-gray-700 opacity-70'
                   }`}
                 >
                   <div className="flex justify-between items-center">
                     <h3 className="font-bold text-sm">{agentType.name}</h3>
                     <span className="text-xs bg-gray-700 px-2 py-1 rounded">
                       {currentCount}/{agentType.maxCount}
                     </span>
                   </div>

                   <div className="text-xs text-gray-300 mt-1 mb-2">
                     <div>Efficiency: {agentType.efficiency.toFixed(2)}</div>
                     <div>Autonomy: {agentType.autonomy.toFixed(2)}</div>
                     <div>Cost: ${formatNumber(agentType.cost)}</div>
                   </div>

                   <button
                     onClick={() => !isFullyAutomated && purchaseAgent(agentType)}
                     disabled={money < agentType.cost || isFullyAutomated || isMaxedOut}
                     className={`w-full py-1 px-3 rounded text-xs transition ${
                       isMaxedOut ? 'bg-gray-600 cursor-not-allowed' :
                       money >= agentType.cost && !isFullyAutomated
                         ? 'bg-blue-600 hover:bg-blue-700'
                         : 'bg-gray-700 cursor-not-allowed'
                     }`}
                   >
                     {isMaxedOut ? 'Max Reached' : 'Purchase'}
                   </button>
                 </div>
               );
             })}
 ...
 >
   <div className="flex justify-between items-center mb-4">
     <div className="text-center flex-grow">
       <h1 className="text-3xl font-bold mb-1">The Last Code</h1>
       <p className="text-gray-400">Coding stops with you. Will you let AIs take over?</p>
       <button
       ...
       <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
         <div>
           ...
           <ul className="list-disc pl-5 text-gray-300">
             ...
             <li><span className="bg-gray-700 px-1 rounded">1-9</span> - Purchase AI Model (when upgrades open)</li>
           </ul>
         </div>
         <div>
           ...
           <ul className="list-disc pl-5 text-gray-300">
             ...
             <li><span className="bg-gray-700 px-1 rounded">1-9</span> - Quick Assign Task to Agent #</li>
           </ul>
         </div>
       </div>
       <div className="mt-2 text-xs text-gray-400">
         Global: <span className="bg-gray-700 px-1 rounded">D</span> - Debug Mode, <span className="bg-gray-700 px-1 rounded">?</span> - Toggle Help
       </div>
     </div>
   )}
...
const purchaseAgent = (agentType) => {
 ...
 // Check if we've reached the max count for this agent type
 const currentCount = getAgentCount(agentType.id);
 if (currentCount >= agentType.maxCount) return;
 ...
 const newAgent = {
   id: `agent-${Date.now()}`,
   type: agentType.id,
   name: `${agentType.name} #${currentCount + 1}`,
...
// Modified assignment logic to handle autonomy levels and verification mode
const assignTask = (task, agent) => {
 ...
 // Check if agent is in verification mode
 if (agentsInVerificationMode.includes(agent.id)) return;

 const agentType = getAgentType(agent.type);

 // Create new assignment with appropriate status
 // Tasks always start working now (per requirements) but may need verification
 const newAssignment = {
...
// Toggle verification mode for an agent
const toggleVerificationMode = (agentId) => {
 if (agentsInVerificationMode.includes(agentId)) {
   // Remove from verification mode
   setAgentsInVerificationMode(prev => prev.filter(id => id !== agentId));
 } else {
   // Add to verification mode
   setAgentsInVerificationMode(prev => [...prev, agentId]);
 }
};

// Helper to count agents of a specific type
const getAgentCount = (agentTypeId) => {
...
const getAvailableAgents = () => {
 return ownedAgents.filter(agent => {
   const agentType = getAgentType(agent.type);
   // Agent must not be in verification mode and must have capacity
   return agent.currentTasks < agentType.maxTasks &&
          !agentsInVerificationMode.includes(agent.id);
 });
};
