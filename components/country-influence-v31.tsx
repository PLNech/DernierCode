      ...
      )}  // Calculate max affordable level for an upgrade
  const getMaxAffordableLevel = (upgrade) => {
    let maxLevel = upgrade.level;
    
    // Check each potential next level
    for (let nextLevel = upgrade.level + 1; nextLevel <= upgrade.maxLevel; nextLevel++) {
      const costPerTrait = upgrade.cost(nextLevel - 1);
      
      // Check if all traits can afford this level
      let canAfford = true;
      ...
      if (canAfford) {
        ...
        // Can't afford this level, so can't afford any higher
        break;
      }
    }
...
const UPGRADES = [
  ...
  useEffect(() => {
    ...
    // Create a continuous stream of news by concatenating news items
    const buildNewsText = () => {
      const newsItems = allNewsItems.map(item => item.text).join(' • ');
      // Add some padding between the end and beginning for continuous scrolling
      setCurrentNewsText(newsItems + ' • ' + newsItems.substring(0, 100));
    };
    
    buildNewsText();
    
    // Rebuild news text when news items change
    const newsInterval = setInterval(() => {
      buildNewsText();
    }, 30000); // Rebuild every 30 seconds to ensure latest news is included
    
    return () => clearInterval(newsInterval);
  }, [allNewsItems]);      <div className="flex flex-col md:flex-row">
        {/* Country Stats Panel */}
        <div className="w-full md:w-1/4 p-4 bg-gray-850 border-r border-gray-800">
          ...
          {/* Horizontal layout for stats: chart on left, table on right */}
        ...
        <div className="w-full md:w-3/4 p-4">
          ...
          {/* Horizontal layout for cards and log */}
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <div className="md:w-1/2">
              <div className="flex flex-row justify-center items-center space-x-4 mb-4">
                {activeCards.map((card, index) => (
                  <div 
                    key={card.id}
                    className={`w-44 h-64 p-3 rounded-lg border-2 transition-all cursor-pointer flex flex-col
                      ${card.style} 
                      ${selectedCardIndex === index ? 'ring-2 ring-yellow-400 scale-105' : 'opacity-80 scale-100'}`}
                    onClick={() => handleCardClick(index)}
                    style={{ aspectRatio: '2/3' }}
                  >
                    <div className="font-bold text-sm mb-1 text-center border-b border-gray-600 pb-1">{card.name}</div>
                    
                    <div className="flex-grow flex flex-col justify-between">
                      <p className="text-xs text-gray-300 mb-2 italic">{card.description}</p>
                      
                      <div className="space-y-2 mb-2">
                        <div className="flex justify-between items-center p-1 bg-black bg-opacity-30 rounded">
                          <span className={`text-xs ${TRAITS_CONFIG[card.bonusTrait].color}`}>
                            {TRAITS_CONFIG[card.bonusTrait].name}
                          </span>
                          <span className="text-green-400 font-bold text-md">+{card.bonusValue}</span>
                        </div>
                        <div className="flex justify-between items-center p-1 bg-black bg-opacity-30 rounded">
                          <span className={`text-xs ${TRAITS_CONFIG[card.malusTrait].color}`}>
                            {TRAITS_CONFIG[card.malusTrait].name}
                          </span>
                          <span className="text-red-400 font-bold text-md">-{card.malusValue}</span>
                        </div>
                      </div>
                      
                      {index === selectedCardIndex && activeCards[selectedCardIndex]?.id === card.id && !card.userSelected && (
                        <div className="mb-1 py-1 text-center text-xs text-yellow-400 bg-yellow-900 bg-opacity-30 rounded animate-pulse">
                          Auto-selected
                        </div>
                      )}
                      
                      {/* Card bottom decoration */}
                      <div className="border-t border-gray-600 pt-1 text-xs text-gray-400 text-center">
            ...
            <div className="md:w-1/2">
              ...
              <div 
                className="bg-gray-800 rounded-lg p-2 h-64 overflow-y-auto text-sm border border-gray-700"
                ref={el => {
                  if (el) {
...
interface Upgrade {
  ...
  level: number;
  maxLevel: number;
  cost: (level: number) => number; // Function that returns cost based on level
  effect: string;
}
...
const CountryInfluenceGame = () => {
  ...
  const drawCards = () => {
    ...
    // Randomly pre-select one card (50% chance for each)
    setSelectedCardIndex(Math.random() < 0.5 ? 0 : 1);
    setTimeRemaining(10);
  };
  ...
  const applyPolicy = (card) => {
    ...
    if (card.news && card.news.length > 0) {
      ...
      // Add to the list of all news items
      setAllNewsItems(prev => [...newsToAdd, ...prev].slice(0, 100)); // Keep only last 100 news items
    }
  ...
  const renderStarChart = () => {
    const centerX = 100;
    const centerY = 100;
    const radius = 70; // Reduced radius to ensure it doesn't get cut off
      ...
      <div className="bg-black text-yellow-400 border-b border-yellow-900 px-3 py-2 flex items-center">
        ...
        <div className="news-ticker flex-1">
          <div className="news-ticker-content" dangerouslySetInnerHTML={{ 
            __html: currentNewsText.replace(/\*([^*]+)\*/g, '<em class="text-white">$1</em>')
          }} />
        </div>
      </div>
